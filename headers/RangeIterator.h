...
for(pair<double *,int *> x : over(vector<double>(10),array<int,20>())){
        //iterates x 10 times through the first 10 items of the vector and the first 10 items of the
array
}
...
for(pair<double *,double *> x : each(Vec.begin(),Vec.end(),Vec.rbegin(),Vec.rend())){
        //iterates x.first 100 times through Vec forwards and x.second 100 times though Vec
backwards
}
*/
#include <utility>
...
struct RangeIterate
{
    struct RangeIterator
    {
        ...
        {
            return (incr < 0) ? (Other.x < x)
                              : (x < Other.x); // incr == 0 is an exception that is handled
        }
  ...
  public:
    class DuoIterate
    {
      public:
        ...
        DuoIterate() = default;
        bool operator!=(DuoIterate& Other) { return It1 != Other.It1 && It2 != Other.It2; }
...
inline auto range(StartType start, EndType end, IncrType incr) -> RangeStuff::RangeIterate<long>
{
...
inline auto over(Cont1 Container1, Cont2 Container2)
  -> RangeStuff::DuoEachIterate<decltype(Container1.begin()), decltype(Container2.begin())>
{
    return RangeStuff::DuoEachIterate<decltype(Container1.begin()), decltype(Container2.begin())>(
