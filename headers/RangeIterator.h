...
for(pair<double *,int *> x : over(vector<double>(10),array<int,20>())){
        //iterates x 10 times through the first 10 items of the vector and the
...
for(pair<double *,double *> x :
each(Vec.begin(),Vec.end(),Vec.rbegin(),Vec.rend())){
        //iterates x.first 100 times through Vec forwards and x.second 100 times
though Vec backwards
}
*/
#include <utility>
...
struct RangeIncr
{
    ...
    {
        Type x;
        bool operator!=(RangeIteratorForward& Other) { return x < Other.x; }
        void operator++() { x++; }
    ...
    RangeIteratorForward& begin() { return Start; }
    RangeIteratorForward& end() { return End; }
};
template<typename Type>
struct RangeIterate
{
    struct RangeIterator
    {
        ...
        bool operator!=(RangeIterator& Other)
        {
            return (incr < 0)
                     ? (Other.x < x)
                     : (x <
                        Other.x); // incr == 0 is an exception that is handled
        }
        void operator++() { (x += incr); }
        Type& operator*() { return x; }
    };
    ...
    RangeIterator& begin() { return Start; }
    RangeIterator& end() { return End; }
};
...
class DuoEachIterate
{
  public:
    ...
    DuoIterate& begin() { return Begin; }
    DuoIterate& end() { return End; }
};
}
template<typename StartType, typename EndType, typename IncrType>
inline auto range(StartType start, EndType end, IncrType incr)
  -> RangeStuff::RangeIterate<long>
{
    return RangeStuff::RangeIterate<long>(start, end, incr);
}
inline auto range(long start, long end) -> RangeStuff::RangeIncr<long>
{
    return RangeStuff::RangeIncr<long>(start, end);
}
inline RangeStuff::RangeIncr<long> range(long end)
{
    return RangeStuff::RangeIncr<long>(0, end);
}
template<typename ItTy>
...
template<typename ItTy1, typename ItTy2>
inline RangeStuff::DuoEachIterate<ItTy1, ItTy2> each(ItTy1 InBegin1,
                                                     ItTy1 InEnd1,
                                                     ItTy2 InBegin2,
                                                     ItTy2 InEnd2)
{
    return RangeStuff::DuoEachIterate<ItTy1, ItTy2>(
      InBegin1, InEnd1, InBegin2, InEnd2);
}
template<typename Cont1, typename Cont2>
inline auto over(Cont1 Container1, Cont2 Container2)
  -> RangeStuff::DuoEachIterate<decltype(Container1.begin()),
                                decltype(Container2.begin())>
{
    return RangeStuff::DuoEachIterate<decltype(Container1.begin()),
                                      decltype(Container2.begin())>(
      Container1.begin(),
      Container1.end(),
      Container2.begin(),
