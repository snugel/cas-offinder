...
enum DeviceType { CPU, GPU, ACCEL };
SearchFactory* create_search_factory(DeviceType device);
int num_searchers_avaliable(SearchFactory* fact);
Searcher* create_searcher(SearchFactory* factory, int searcher_idx, uint64_t max_genome_size,
                          uint64_t max_matches, uint8_t* bit4patterns, uint64_t num_patterns,
