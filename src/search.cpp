...
int num_searchers_avaliable(SearchFactory* fact) { return fact->devices.size(); }
...
Searcher* create_searcher(SearchFactory* fact, int searcher_idx, uint64_t max_matches,
                          uint64_t max_genome_size, uint8_t* bit4patterns, uint64_t num_patterns,
    ...
    searcher->genome_buf = oclCreateBuffer(searcher->context, CL_MEM_READ_WRITE,
                                           num_bytes(searcher, max_genome_size), nullptr);
    ...
    oclEnqueueWriteBuffer(searcher->queue, searcher->pattern_buf, CL_TRUE, 0,
                          padded_patterns.size(), padded_patterns.data(), 0, 0, 0);
...
void search(Searcher* searcher, uint8_t* bit4genome, uint64_t genome_size, uint32_t max_mismatches,
            Match** match_result, uint64_t* num_matches) {
    oclSetKernelArg(searcher->search, 2, sizeof(cl_uint), &max_mismatches);
    oclEnqueueWriteBuffer(searcher->queue, searcher->genome_buf, CL_TRUE, 0, cdiv(genome_size, 2),
                          bit4genome, 0, 0, 0);
    int zero = 0;
    oclEnqueueWriteBuffer(searcher->queue, searcher->count_buf, CL_TRUE, 0, sizeof(zero), &zero, 0,
                          0, 0);
    ...
    cl_uint count;
    oclEnqueueReadBuffer(searcher->queue, searcher->count_buf, CL_TRUE, 0, sizeof(count), &count, 0,
                         0, 0);
    if (count > 0) {
        *num_matches = count;
        *match_result = (Match*)malloc(count * sizeof(Match));
        oclEnqueueReadBuffer(searcher->queue, searcher->match_buf, CL_TRUE, 0,
                             sizeof(Match) * count, *match_result, 0, 0, 0);
